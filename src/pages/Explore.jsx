    ...
    <div className={`min-h-screen pt-20 ${theme === 'dark' ? 'bg-[#0A0A0A]' : 'bg-white'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className={`md:hidden flex items-center gap-2 px-4 py-2 rounded-lg ${
                theme === 'dark' ? 'bg-[#1A1A1A] text-white' : 'bg-gray-100 text-gray-700'
              }`}
            >
        ...
        <div className="flex gap-8">
          <div className={`hidden md:block w-64 flex-shrink-0 ${showFilters ? 'block' : ''}`}>
          ...
          {showFilters && (
            <div className="md:hidden fixed inset-0 z-40 bg-black/50" onClick={() => setShowFilters(false)}>
              <div className={`absolute right-0 top-0 h-full w-80 ${theme === 'dark' ? 'bg-[#0A0A0A]' : 'bg-white'} p-6 overflow-y-auto`}>
                <div className="flex items-center justify-between mb-6">
                  <h2 className={`font-semibold ${theme === 'dark' ? 'text-white' : 'text-[#01172F]'}`}>Filters</h2>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <label className="flex items-center gap-2">
                        <input type="radio" name="mobile-cat" checked={selectedCategory === ''} onChange={() => setSelectedCategory('')} />
                        <span>All</span>
                      </label>
                      {categories.map(cat => (
                        <label key={cat.id} className="flex items-center gap-2">
                          <input type="radio" name="mobile-cat" checked={selectedCategory === cat.id} onChange={() => setSelectedCategory(cat.id)} />
                          <span>{cat.name}</span>
                        </label>
                      ))}
                    </div>
                ...
                <button onClick={() => setShowFilters(false)} className="w-full mt-6 py-3 bg-[#01172F] text-white rounded-xl">
                  Apply Filters
                </button>
              </div>
            </div>
